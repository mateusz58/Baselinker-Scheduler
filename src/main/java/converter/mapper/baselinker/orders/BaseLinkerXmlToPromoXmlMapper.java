...
public class BaseLinkerXmlToPromoXmlMapper implements Mapper<OrderBaseLinkerXml, OrderPromo> {
  ...
  public OrderPromo map(OrderBaseLinkerXml orderBaseLinker) throws ParseException {
  ...
  @Override
  public List<OrderPromo> map(List<OrderBaseLinkerXml> orderBaseLinker) throws ParseException {
    List<OrderPromo> orderPromos = new LinkedList<>();
    for (int i = 0; i < orderBaseLinker.size(); i++) {
      orderPromos.add(map(orderBaseLinker.get(i)));
    }
    return orderPromos;
  }
}
